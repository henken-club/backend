...
export class UsersResolver {
  ...
  async resolveFollowees(
    ...
    @Args({ type: () => FolloweesArgs }) { orderBy, ...pagination }:
      FolloweesArgs,
  ...
  async resolveFollowers(
    @Parent() { id }: UserEntity,
    @Args({ type: () => FollowersArgs }) { orderBy, ...pagination }:
      FollowersArgs,
  ): Promise<FollowingConnectionEntity> {
  ...
  async resolvePostsHenkens(
    @Parent() { id }: UserEntity,
    @Args({ type: () => PostsHenkensArgs }) { orderBy, filter, ...pagination }:
      PostsHenkensArgs,
  ...
  async resolvePostsAnswers(
    @Parent() { id }: UserEntity,
    @Args({ type: () => PostsAnswersArgs }) { orderBy, filter, ...pagination }:
      PostsAnswersArgs,
  ...
  async registerUser(
    @Viewer() { accountId }: ViewerType,
    @Args({ type: () => RegisterUserArgs }) args: RegisterUserArgs,
